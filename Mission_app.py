...
elevation_cache = {}
def fetch_elevations(coords, config):
    elevs = []
    ...
    if elevs and any(e > -500 for e in elevs):
...
def generate_takeoff_and_cruise(home, first_survey_point, config):
    ...
    item_id_counter += 1
    wp7_xy = move_point_along_bearing(entry, (brg_to_entry + 180) % 360, config["loiter_distance"])
    ...
    item_id_counter += 2
    coord_list = [first_survey_point]
    elevations = fetch_elevations(coord_list, config=config)
    cruise_alt = elevations[0] + safety_margin
...
def generate_simplified_path(lines, home_pt, rtl_pt, config):
    ...
    for i, seg in enumerate(lines):
        ...
        ev = fetch_elevations(seg_coords, config=config)
